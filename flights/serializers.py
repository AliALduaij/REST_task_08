...
class ProfileSerializer(serializers.ModelSerializer):
	...
	def get_past_bookings(self,obj):
		user_obj = obj.user
		booking_list=Booking.objects.filter(user=user_obj,date__lt=date.today())
		#booking_list=user_obj.bookings.filter(date__lt=datetime.date.today) << another way to use the objects
		return BookingSerializer(booking_list,many=True).data #<< .data to collect the data inside the serializer
